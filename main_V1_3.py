...
def oil_detection(image, path, num, save_path, view_img=False, save_image=True):
    ...
    if save_image:
        ...
        cv2.imwrite(str(save_path) + f"/{path}_Homomorphic{num + 1}.png", negative_image_cv)
    
    oil_img = new_image.copy()
    x, y = oil_img.shape
    # print(oil_img.shape)
    ...
    def run(self, 
        ...
        for _, result in enumerate(results):
            ...
            for b, (_, conf, cutimg) in enumerate(zip(xyxy_arr, conf_arr, cutimgs)):
                ...
                """ 油花檢測 """
                # black_area_size, rate1 = oil_detection(cutimg, "", b, save_path, view_img=False, save_image=True)
                ...
                csv.write_data([b, t_value_black_area_size[0], round(t_value_rate1[0]*100,2), conf, t_value_lmean[0]])
                ...
                # 順序, 油花等級, 肉色等級, 最終等級
                return_results_arr.append([b + 1, oil_level, color_level, final_level, title, ingredients, steps])
